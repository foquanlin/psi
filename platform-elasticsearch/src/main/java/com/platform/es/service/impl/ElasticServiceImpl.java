package com.platform.es.service.impl;

import com.google.gson.Gson;
import com.platform.es.service.ElasticSearchService;
...
//                T result = objectMapper.readValue(sourceAsString, mappingClass);
                T result = gson.fromJson(sourceAsString, mappingClass);
                return result;
            }
        } catch (Exception e) {
    ...
    public boolean batchInsertDocument(String index, List<?> list, XContentType dataType) {
        ...
        for (Object obj : list) {
            // 自动生成id
            bulkRequest.add(new IndexRequest(index).source(gson.toJson(obj), dataType));
        }
