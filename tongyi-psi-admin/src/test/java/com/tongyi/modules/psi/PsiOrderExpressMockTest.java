/*
 * 项目名称:项目名称
 * 类名称:PsiOrderExpressMockTest.java
...
import com.tongyi.modules.psi.controller.PsiOrderExpressController;
...
public class PsiOrderExpressMockTest {
    @Autowired
    ...
    @Autowired
    protected PsiOrderExpressController controller; // 把要测试的controller注入进来
    ...
    public void testAdd() {
        for(int i=0;i<100;i++) {
            ...
            String id = "id-" + idx;
            PsiOrderExpressEntity item = new PsiOrderExpressEntity();
            item.setId(id);
            item.setOrderId (id);
            item.setType (id);
