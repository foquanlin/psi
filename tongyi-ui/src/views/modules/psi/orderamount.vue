  ...
  <div class="mod-orderamount">
    <el-form :inline="true" :model="searchForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('psi:orderamount:save')" type="primary" @click="editHandle()">新增</el-button>
        <el-button v-if="isAuth('psi:orderamount:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table border :data="dataList" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template v-slot="scope">
          <el-button v-if="isAuth('psi:orderamount:info')" type="text" size="small" @click="showDetails(scope.row.id)">查看</el-button>
          <el-button v-if="isAuth('psi:orderamount:update')" type="text" size="small" @click="editHandle(scope.row.id)">修改</el-button>
          <el-button v-if="isAuth('psi:orderamount:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <orderamount-edit v-if="editVisible" ref="orderamountEdit" @refreshDataList="getDataList"/>
  </div>
</template>

<script>
  import orderamountEdit from './orderamount-edit'
  ...
  export default {
    ...
    methods: {
      ...
      showDetails (id) {
        ...
        this.$nextTick(() => {
          this.$refs.orderamountEdit.init(id, true)
        })
      },
      // 新增 / 修改
      editHandle (id) {
        this.editVisible = true
        this.$nextTick(() => {
          this.$refs.orderamountEdit.init(id)
        })
      },
      // 删除
      deleteHandle (id) {
