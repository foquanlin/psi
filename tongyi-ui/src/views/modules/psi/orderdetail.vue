<template>
  <div class="mod-orderdetail">
    ...
    <el-table border :data="dataList" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="price" header-align="center" align="center" label="进货价"/>
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button v-if="isAuth('psi:orderdetail:info')" type="text" size="small" @click="showDetails(scope.row.id)">查看</el-button>
          <el-button v-if="isAuth('psi:orderdetail:update')" type="text" size="small" @click="editHandle(scope.row.id)">修改</el-button>
          <el-button v-if="isAuth('psi:orderdetail:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <orderdetail-edit v-if="editVisible" ref="orderdetailEdit" @refreshDataList="getDataList"/>
  </div>
</template>

<script>
  import orderdetailEdit from './orderdetail-edit'
  ...
  export default {
    ...
    methods: {
      ...
      showDetails (id) {
        ...
        this.$nextTick(() => {
          this.$refs.orderdetailEdit.init(id, true)
        })
      },
      // 新增 / 修改
      editHandle (id) {
        this.editVisible = true
        this.$nextTick(() => {
          this.$refs.orderdetailEdit.init(id)
        })
      },
      // 删除
      deleteHandle (id) {
