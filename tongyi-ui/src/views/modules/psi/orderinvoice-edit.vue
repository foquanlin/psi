<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :inline="true" :rules="dataRule" ref="dataForm" label-width="120px" @keyup.enter.native="dataFormSubmit()">
      ...
      <el-form-item label="发票" prop="attachmentUrls">
        <el-input v-model="dataForm.attachmentUrls" :disabled="disabled" placeholder="发票" clearable/>
      </el-form-item>
    </el-form>
    ...
    methods: {
      init (id, disabled) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: `/psi/orderinvoice/info/${this.dataForm.id}`,
              method: 'get'
            }).then(({data}) => {
              if (data && data.code === 0) {
      ...
      dataFormSubmit () {
        this.$refs['dataForm']
          .validate((valid) => {
            if (valid) {
              this.$http({
                url: `/psi/orderinvoice/${!this.dataForm.id ? 'save' : 'update'}`,
                method: 'post',
                data: this.dataForm
              }).then(({data}) => {
                if (data && data.code === 0) {
