  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :inline="true" :rules="dataRule" ref="dataForm" label-width="120px" @keyup.enter.native="dataFormSubmit()">
      ...
      <el-form-item label="说明" prop="desc">
        <el-input v-model="dataForm.desc" :disabled="disabled" placeholder="说明" clearable/>
      </el-form-item>
      ...
      <el-form-item label="操作人" prop="operationUid">
        <el-input v-model="dataForm.operationUid" :disabled="disabled" placeholder="操作人" clearable/>
      </el-form-item>
    </el-form>
    ...
    methods: {
      init (id, disabled) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: `/psi/orderoperation/info/${this.dataForm.id}`,
              method: 'get'
            }).then(({data}) => {
              if (data && data.code === 0) {
      ...
      dataFormSubmit () {
        this.$refs['dataForm']
          .validate((valid) => {
            if (valid) {
              this.$http({
                url: `/psi/orderoperation/${!this.dataForm.id ? 'save' : 'update'}`,
                method: 'post',
                data: this.dataForm
              }).then(({data}) => {
                if (data && data.code === 0) {
