...
export default {
  data () {
    return {
      msg: {},
      timeSelections: {
        '近24小时': this.$moment().days(0).format(TIME_FORMAT),
        '近3天': this.$moment().days(-3).format(TIME_FORMAT),
        '近7天': this.$moment().days(-7).format(TIME_FORMAT),
        '近30天': this.$moment().days(-30).format(TIME_FORMAT)
      },
      dataForm: {
        startTime: this.$moment().days(-1).format(TIME_FORMAT),
        msgTypes: ''
      },
      dataList: [],
      userDataList: [],
